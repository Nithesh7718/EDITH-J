...
/**
 * A JavaFX {@link Canvas} that renders an EDITH-style circular audio aura.
 *
 * <p>
 * The aura is composed of:
 * <ul>
 * <li>A centre avatar glow circle with an EDITH logo.</li>
 * <li>N radial "spike" bars whose height is modulated by
 * {@link #audioLevelProperty()}.</li>
 * <li>Two outer glow rings that pulse according to
 * {@link #uiStateProperty()}.</li>
 * </ul>
 *
 * <p>
 * CPU usage is kept low by drawing on an
 * {@link javafx.animation.AnimationTimer}-driven internal loop that only
 * redraws when the canvas is visible.
 */
public final class AudioAuraCanvas extends Canvas {
    ...
    /**
     * Current audio input level in [0, 1]. Bind to your microphone volume
     * meter.
     */
    ...
    // ── Internal animation ───────────────────────────────────────────────────
    /**
     * Animated amplitude multiplier for the breathing / pulse effect (0–1
     * range).
     */
    ...
    // ── Public API ────────────────────────────────────────────────────────────
    public DoubleProperty audioLevelProperty() {
        return audioLevel;
    }

    public double getAudioLevel() {
        return audioLevel.get();
    }

    public void setAudioLevel(double v) {
        audioLevel.set(Math.max(0, Math.min(1, v)));
    }
    ...
    public void setUiState(AssistantUiState s) {
        uiState.set(s);
    }
    ...
    private void drawAvatar(GraphicsContext gc, double cx, double cy,
        ...
        RadialGradient glowPaint = new RadialGradient(
            ...
            case IDLE ->
                Color.web("#1FD5F9", 0.20);
            case LISTENING ->
                Color.web("#1FD5F9", 0.45);
            case PROCESSING ->
                Color.web("#7B61FF", 0.35);
            case SPEAKING ->
                Color.web("#00FFA3", 0.40);
        }),
